...
- **알려진 이슈 및 한계점**: A* 길찾기 알고리즘이 100x100 대형 맵에서 잦은 빈도로 연산될 경우 발생할 수 있는 프레임 지연을 막기 위해 최대 탐색 깊이(Iteration Limit)를 200으로 강제 제한함. 이로 인해 극도로 복잡한 미로 구조에서는 타겟을 포기하고 배회 상태로 빠질 수 있음.
